...
public class GeneTransFormUtils {
	...
	public static boolean match(JSONObject matchJson, JSONObject geneJson) {
		...
		for (Map.Entry<String, Object> entry : matchJson.entrySet()) {
			...
			if (geneJson.getString(key) != null) {
				List<String> value = new ArrayList<>(Arrays.asList(matchJson.getString(key).split("/")));
				List<String> gene = new ArrayList<>(Arrays.asList(geneJson.getString(key).split("/")));
				if (value.size() != value.stream().filter(gene :: contains).collect(Collectors.toList()).size()) {
					flag = false;
					break;
				}
			}
	 ...
	 * @return Map
	 */
	public static Map<String, Integer> matchGeneContanins(JSONObject matchJson, JSONObject geneJson) {
		int diff = 0;
		int trim = 0;
		int m = 0;
		for (String key : matchJson.keySet()) {
			if (geneJson.getString(key) != null) {
				Set<String> value = new HashSet<>(Arrays.asList(matchJson.getString(key).split("/")));
				Set<String> gene = new HashSet<>(Arrays.asList(geneJson.get(key).toString().split("/")));
