package com.bazl.dna.monitor.deploy;
...
public class DeployMethod {
	...
	public static void deployServer(List<DeployServer> deployServerList,
		...
		try {
			...
			for (DeployServer server : deployServerList) {
				...
				if (auth) {
					List<String> cmds = deploy(con, server, projectNames, local, targetRoot, version, suffix);
	...
	public static List<String> deploy(Connection con, DeployServer server,
		...
		for (String projectName : projectNames) {
			...
			String targetPath = targetRoot + FILE_DELIMITER + projectName + FILE_DELIMITER;
			if (DeployUtils.isMac()) {
				// mac scp方式
			...
			// 准备执行的命令
			cmds.add(targetRoot + File.separator + projectName + File.separator + "start.sh");
		}
		return cmds;
	}
}
