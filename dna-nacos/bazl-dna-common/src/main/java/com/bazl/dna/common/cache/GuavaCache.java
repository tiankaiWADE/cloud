...
public class GuavaCache<T> {
	...
	/** 创建缓存 */
	private LoadingCache<String, Optional<T>> cache = CacheBuilder.newBuilder()
			.concurrencyLevel(CONCURRENCY_LEVEL)
			.expireAfterWrite(EXPIRE_AFTER_WRITE, TimeUnit.MINUTES)
			.initialCapacity(INITIAL_CAPACITY)
			.maximumSize(MAXIMUM_SIZE)
			.recordStats()
			.removalListener(new RemovalListener<String, Optional<T>>() {
				@Override
				public void onRemoval(RemovalNotification<String, Optional<T>> notification) {
					LOGGER.debug("removed cause: {}", notification.getCause());
				}
			})
