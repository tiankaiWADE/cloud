package com.bazl.dna.files.client;
...
public class FastdfsClient {
	...
	private void saveFastdfsFile(String url, String fileName, long fileSize) {
		...
		fastdfsFile.setId(StringUtils.replace(url, fastdfsFile.getFastdfsUrl() + "/", ""));
		fastdfsFile.setCreateTime(new Date());
		fastdfsFile.setDirectory(false);
		fastdfsFile.setFilePath(url);
		fastdfsFile.setFileName(StringUtils.substringAfterLast(url, "/"));
	...
	private String getExtName(String fileName, String contentType) {
    	...
    	if ("".equals(result)) {
    		return StringUtils.substringAfterLast(contentType, "/");
    	}
    	return result;
    }
	...
	public byte[] download(String fileUrl) {
		...
		String downloadPath = fileUrl.substring(fileUrl.indexOf('/') + 1);
