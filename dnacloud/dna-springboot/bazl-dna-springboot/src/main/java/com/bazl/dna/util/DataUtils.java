...
public class DataUtils {
	...
	public static final char C_DELIMITER = '/';
	public static final String C_QUATE = ",";
	...
	public static String getFileName(String f) {
		if (f != null) {
			String fileName = f.replace('\\', C_DELIMITER);
			if (fileName.lastIndexOf(C_DELIMITER) > 0) {
				fileName = fileName.substring(fileName.lastIndexOf(C_DELIMITER) + 1);
			}
			int i = fileName.lastIndexOf('.');
			if (i > 0 && i < fileName.length() - 1) {
			...
			if (delimIndex == -1) {
				...
				if (delimIndex > 0 && strPattern.charAt(delimIndex - 1) == C_BACKSLASH) {
					if (delimIndex > 1 && strPattern.charAt(delimIndex - 2) == C_BACKSLASH) {
						...
						sbuf.append(ConvertString.utf8Str(argArray[argIndex]));
	...
	public static String getIpAddress(HttpServletRequest request) {
		...
		if (ip != null && ip.length() > IP_LENGTH && ip.contains(C_QUATE)) {
			ip = StringUtils.substringAfterLast(ip, C_QUATE);
		}
		return ip;
	}
